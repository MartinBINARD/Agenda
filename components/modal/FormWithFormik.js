...
export default function FormWithFormik({ isFormVisible, closeForm, selectedEvent }) {
    ...
    const validationSchema = Yup.object().shape({
        ...
        startDate: Yup.date(),
        endDate: Yup.date().min(Yup.ref('startDate'), "La fin de l'événement doit être après le début"),
    });
        ...
        <Modal visible={isFormVisible} presentationStyle="formSheet" animationType="slide">
            <Pressable style={styles.formContainer} onPress={closeKeyboardHandler}>
                ...
                <Formik initialValues={initialState} onSubmit={onSubmit} validationSchema={validationSchema}>
                    {({ values, handleChange, setFieldValue, handleSubmit, errors, touched, status, setStatus, isSubmitting }) => {
                        ...
                        return (
                            <>
                                ...
                                />
                                <IsOnline isEnabled={values.isOnline} setIsEnabled={(value) => setFieldValue('isOnline', value)} />
