...
export default function Form({ isFormVisible, closeForm, selectedEvent }) {
    const event = useSelector((state) => state.agenda.events.find((event) => event.id === selectedEvent));
        ...
        <Modal visible={isFormVisible} presentationStyle="formSheet" animationType="slide">
            <Pressable style={styles.formContainer} onPress={closeKeyboardHandler}>
                <View style={styles.headerContainer}>
                    ...
                    <Feather name="trash-2" size={28} color={colors.LIGHT} onPress={removeEvt} suppressHighlighting={true} />
                </View>
                <Input
                    label="Titre"
                    autoCorrect={false}
                ...
                />
                <DateTimePicker label="Début" dateTime={formData.startDate.value} setDateTime={onFormChange.bind(this, 'startDate')} />
                <DateTimePicker
                    label="Fin"
                ...
                />
                <IsOnline isEnabled={formData.isOnline.value} setIsEnabled={onFormChange.bind(this, 'isOnline')} />
